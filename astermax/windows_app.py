...
The GUI is intentionally thin: it orchestrates the already verified engineering
kernels instead of duplicating solver logic.  It lets an engineer import a STEP file,
validate the mm invariant, define a first linear-static case, mesh with Gmsh, solve,
write auditable evidence and open the self-contained result viewer.

This is a PMV desktop shell, not a claim of full commercial-CAE feature parity.
"""
...
def find_gmsh_for_windows(explicit: str | None = None) -> str:
    ...
    candidates.extend((
        exe_dir / "tools" / "gmsh" / "gmsh.exe",
        exe_dir / "gmsh" / "gmsh.exe",
    ))
...
def run_windows_step_case(config: StepCaseConfig, *, gmsh_executable: str | None = None) -> dict:
    """Execute the verified real-STEP static pipeline from a desktop case definition."""
    config.validate()
    validate_step_mm_file(config.step_path)
...
def launch_desktop() -> int:
    ...
    ttk.Label(header, text="PMV / Windows · STEP → Mesh → FEA → Evidence", font=("Segoe UI", 11)).pack(side="left", padx=14)
    ...
    notebook.add(setup, text="2  Model Setup")
    ...
    ttk.Label(geometry, text="Current PMV preparation: semantic persistent boundary intent. FIXED/LOAD surfaces are re-identified after meshing instead of relying on fragile CAD face IDs.", wraplength=900).grid(row=5, column=0, sticky="w", pady=(24,0))
    ...
    ttk.Label(setup,text="The first desktop case is linear-elastic TET4. Contact, GAP, friction and pretension kernels remain in AsterMax but are not silently applied to arbitrary imported STEP assemblies.",wraplength=900).grid(row=10,column=0,columnspan=4,sticky="w",pady=(24,0))

    ttk.Label(solve_tab, text="Verified solve pipeline", font=("Segoe UI", 15, "bold")).pack(anchor="w")
    ttk.Label(solve_tab, text="STEP mm gate → OpenCASCADE/Gmsh → TET4 quality gate → semantic BC/load → static solve → equilibrium/residual → Von Mises → VTK/HTML → SHA-256 evidence", wraplength=940).pack(anchor="w", pady=(6,14))
    ...
    def solve_worker(config: StepCaseConfig) -> None:
        try:
            result = run_windows_step_case(config)
    ...
    def solve_case() -> None:
        ...
        if source is None:
            messagebox.showwarning("AsterMax", "Import and validate a STEP file first.")
            notebook.select(geometry); return
        try:
            force=_parse_force((fx_var,fy_var,fz_var))
            output=source.parent / f"{source.stem}_AsterMax_Evidence"
            config=StepCaseConfig(
                step_path=source, output_dir=output, axis=axis_var.get(),
                fixed_side=fixed_side_var.get(), load_side=load_side_var.get(),
                force_n=force, mesh_size_mm=float(mesh_var.get()),
        ...
        log(f"Gmsh: {gmsh}")
        log(f"Case: {source.name} | axis={config.axis} FIXED={config.fixed_side} LOAD={config.load_side}")
        log(f"Force [N]={config.force_n} mesh [mm]={config.mesh_size_mm} E [MPa]={config.young_mpa} nu={config.poisson}")
        threading.Thread(target=solve_worker,args=(config,),daemon=True).start()
    ...
    ttk.Label(footer,text="mm · N · MPa | fail-closed engineering gates",font=("Segoe UI",9)).pack(side="right")
