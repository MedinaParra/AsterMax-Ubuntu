...
legacy strict plane projection and finite-TRI3 ``closest_feature`` association at
edges/vertices. This is an auditable updated-geometry/Picard verification increment,
...
class UpdatedSurfaceContactState:
    slave_node:int; master_nodes:tuple[int,int,int]; signed_gap_mm:float; penetration_mm:float; normal_force_n:float; active:bool; barycentric:tuple[float,float,float]; normal:tuple[float,float,float]; slave_force_n:tuple[float,float,float]; master_nodal_forces_n:tuple[tuple[float,float,float],...]
@dataclass(frozen=True)
class UpdatedSurfaceContactResult:
    displacements:tuple[float,...]; reactions:tuple[float,...]; residual:tuple[float,...]; contact_states:tuple[UpdatedSurfaceContactState,...]; unmatched_slave_nodes:tuple[int,...]; iterations:int; converged:bool; master_switch_count:int
@dataclass(frozen=True)
class _Candidate:
    slave:int; master:tuple[int,int,int]; gap:float; barycentric:tuple[float,float,float]; normal:tuple[float,float,float]; active:bool
def _unit(v):
        ...
        for tri in oriented:
            ...
            try:
                if projection_mode=="strict":
                    ...
                    p=closest_point_to_triangle(points[slave],*(points[i] for i in tri)); delta=_sub(points[slave],p.projected_point_mm); distance=_norm(delta); side=1.0 if _dot(p.normal,delta)>=0 else -1.0; normal=p.normal if distance<=1e-15 else tuple(side*x/distance for x in delta); gap=side*distance
            except (SurfaceContactError,IndexError) as exc: raise UpdatedSurfaceContactError(str(exc)) from exc
            if distance<=search_distance: candidates.append((distance,tri,p,normal,gap))
        if not candidates: unmatched.append(slave); continue
        _,tri,p,normal,gap=min(candidates,key=lambda item:(item[0],item[1])); found.append(_Candidate(slave,tri,gap,p.barycentric,normal,gap < -activation_tol))
...
def solve_updated_surface_contact_from_stiffness(nodes,stiffness,constraints:Mapping[int,float],loads:Mapping[int,float],*,slave_nodes,master_triangles,master_normal_hint,penalty_stiffness_n_per_mm,search_distance_mm,max_iterations=30,activation_tolerance_mm=1e-10,geometry_tolerance_mm=1e-9,allow_unmatched=False,projection_mode="strict"):
    ...
    for iteration in range(1,max_iterations+1):
        cand,unmatched=_search(_deformed(nodes,u),slaves,masters,hint,search,activation_tolerance_mm,projection_mode); _require_matches(unmatched,allow_unmatched); current={c.slave:c.master for c in cand}
        ...
        un=_solve_linear(ke,fe,fixed); ncand,nun=_search(_deformed(nodes,un),slaves,masters,hint,search,activation_tolerance_mm,projection_mode); _require_matches(nun,allow_unmatched); sig=tuple((c.slave,c.master,c.active) for c in cand); nsig=tuple((c.slave,c.master,c.active) for c in ncand); delta=max(abs(a-b) for a,b in zip(un,u)); u=un
        if nsig==sig and delta<=geometry_tolerance_mm: break
    else:
        fc,um=_search(_deformed(nodes,u),slaves,masters,hint,search,activation_tolerance_mm,projection_mode); _require_matches(um,allow_unmatched); return UpdatedSurfaceContactResult(tuple(u),tuple(0.0 for _ in range(ndof)),tuple(float('nan') for _ in range(ndof)),_states(fc,kp),um,max_iterations,False,switches)
...
def _states(candidates,kp):
    out=[]
    for c in candidates:
        pen=max(0.0,-c.gap) if c.active else 0.0; fn=kp*pen; sf=tuple(fn*x for x in c.normal); mf=tuple(tuple(-w*x for x in sf) for w in c.barycentric); out.append(UpdatedSurfaceContactState(c.slave,c.master,c.gap,pen,fn,c.active,c.barycentric,c.normal,sf,mf))
    return tuple(out)
def solve_tet4_with_updated_surface_contact(nodes,elements,young,poisson,constraints,loads,**contact_kwargs): return solve_updated_surface_contact_from_stiffness(nodes,assemble_stiffness(nodes,elements,young,poisson),constraints,loads,**contact_kwargs)
