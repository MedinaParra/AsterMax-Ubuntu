...
STEP = Path("step_shoulder_sector_probe_convergence.step")
OUT = Path("step_shoulder_sector_probe_convergence.json")
...
def _write_fixture() -> None:
    ...
    try:
        ...
        gmsh.model.add("step_shoulder_sector_probe_convergence")
...
def _solve_sample(feature, ends, material, local_size: float):
    ...
    if probe.covered_sector_count != SECTOR_COUNT or probe.angular_coverage_fraction != 1.0:
        raise RuntimeError("sector probe did not achieve full declared angular coverage")
    ...
    return mesh, applied, result, neighborhood, probe, sample


def main() -> int:
    _write_fixture()
    feature = recognize_x_axis_shaft_shoulder(STEP, feature_id="REAL_SECTOR_PROBE_SHOULDER")
    ...
    for local_size in LOCAL_TARGETS_MM:
        ...
        if sample.force_residual_n > 1.0e-5 or sample.moment_residual_nmm > 1.0e-3:
            raise RuntimeError(
                f"equilibrium failed at local_size={local_size}: force={sample.force_residual_n}, moment={sample.moment_residual_nmm}"
            )
        ...
        per_mesh.append(
            {
                ...
                "probe_sector_count": probe.sector_count,
                "probe_covered_sector_count": probe.covered_sector_count,
                "probe_angular_coverage_fraction": probe.angular_coverage_fraction,
                "probe_ring_mean_mpa": probe.mean_von_mises_mpa,
                "probe_ring_max_mpa": probe.max_von_mises_mpa,
                "probe_ring_max_distance_mm": probe.maximum_sample_distance_mm,
    ...
    singularity = diagnose_local_singularity(
        diagnostic_id="REAL_FINITE_FILLET_SECTOR_PROBE_REFINEMENT",
        samples=tuple(
    ...
    report = {
        "schema": "AsterMaxStepShoulderSectorProbeConvergenceBenchmarkV1",
        "classification": "REAL_TET10_LOCAL_STRESS_SECTOR_PROBE_STUDY_NOT_INDUSTRIAL_RESULT",
        ...
        "measurement_change_from_c7": {
            "reason": "C7 nearest-N probe lacked guaranteed azimuthal coverage",
            "c7_result_preserved": True,
            "same_mesh_sequence": True,
            "same_convergence_policy": True,
            "sector_count": SECTOR_COUNT,
            "required_angular_coverage_fraction": 1.0,
        },
        "predeclared_mesh_sequence": {
            "global_target_mm": GLOBAL_TARGET_MM,
            "local_targets_mm": list(LOCAL_TARGETS_MM),
            "declared_before_c8_result_evaluation": True,
        },
        "geometry": {
        ...
        "interpretation_boundary": (
            "This C8 study changes only the physical probe measurement definition relative to C7: one real TET10 integration point per equal azimuth sector with full 360-degree coverage. "
            "Geometry, load, material, mesh sequence and convergence thresholds are unchanged. A workflow PASS does not imply a convergence claim. No Shigley data, industrial validation or ANSYS equivalence is claimed."
        ),
    }
