...
def tet10_edge_ratio_proxy(nodes_mm: np.ndarray, elements: np.ndarray) -> np.ndarray:
    """Return min/max straight-corner edge ratio per TET10.

    This is deliberately a simple geometric proxy, not ANSYS Element Quality,
    scaled Jacobian, or a solver-equivalent distortion metric.
    """
...
def _oblique_projection(points_mm: np.ndarray) -> np.ndarray:
    ...
    # Deterministic engineering-style oblique view. X remains visually dominant
    # so X_MIN/X_MAX scopes cannot collapse on top of each other as in a YZ view.
    u = p[:, 0] + 0.35 * p[:, 1]
    v = p[:, 2] + 0.20 * p[:, 1]
...
def build_visual_model_preparation_snapshot(
    *,
    ...
    role_map = {"X_MIN": "SUPPORT", "X_MAX": "LOAD", "Y_MIN": "BODY", "Y_MAX": "BODY", "Z_MIN": "BODY", "Z_MAX": "BODY"}
    for group in required:
        ...
        tri_sample = _sample_tri6(tri_all, max_faces_per_group)
        for conn in tri_sample:
            corners = nodes[conn[:3]]
            projected = _oblique_projection(corners)
            face_rows.append(
                {
                    "group": group,
                    "role": role_map[group],
    ...
    # Normalize only for presentation; the physical provenance remains in C4.3.
    ...
    core = {
        "schema": "AsterMaxVisualModelPreparationV1",
        "step_sha256": str(preparation["step_sha256"]),
        "preparation_snapshot_sha256": str(preparation["snapshot_sha256"]),
        "constraint_selection_sha256": str(preparation["constraint_selection_sha256"]),
        "load_selection_sha256": str(preparation["load_selection_sha256"]),
        "support_tri6_count": int(np.asarray(surface_triangles["X_MIN"]).shape[0]),
        "load_tri6_count": int(np.asarray(surface_triangles["X_MAX"]).shape[0]),
        "body_tri6_count": int(sum(np.asarray(surface_triangles[name]).shape[0] for name in required[2:])),
...
def install_visual_model_preparation_tab(notebook: Any) -> Callable[[dict[str, Any]], None]:
    """Install a native visual inspector and return a strict snapshot binder."""
    ...
    ttk.Label(
        panel,
        text="Native diagnostic view of real TRI6 support/load scopes plus C4.3 Jacobian evidence. Edge ratio is a geometric proxy only, not ANSYS Element Quality.",
        wraplength=860,
    ...
    def bind(payload: dict[str, Any]) -> None:
        ...
        canvas.create_text(12, 12, anchor="nw", fill="#45a3ff", text="SUPPORT · X_MIN")
        canvas.create_text(12, 30, anchor="nw", fill="#ff8a45", text="LOAD · X_MAX")
        ...
        lines.extend([
            "",
            "Boundary",
            "Not ANSYS Element Quality. Not scaled Jacobian. No arbitrary-model convergence claim.",
