...
const diag=(()=>{{let lo=[Infinity,Infinity,Infinity],hi=[-Infinity,-Infinity,-Infinity];for(const p of P)for(let k=0;k<3;k++){{lo[k]=Math.min(lo[k],p[k]);hi[k]=Math.max(hi[k],p[k]);}}return Math.hypot(hi[0]-lo[0],hi[1]-lo[1],hi[2]-lo[2]);}})();const maxU=Math.max(...D.deformation.U_MAG_mm,0);
...
function points(scale){{const out=P.map((p,i)=>[p[0]+U[i][0]*scale,p[1]+U[i][1]*scale,p[2]+U[i][2]*scale]);let center=[0,0,0];for(const p of out)for(let k=0;k<3;k++)center[k]+=p[k]/out.length;return out.map(p=>{{let x=p[0]-center[0],y=p[1]-center[1],z=p[2]-center[2];const cy=Math.cos(yaw),sy=Math.sin(yaw),cp=Math.cos(pitch),sp=Math.sin(pitch);const x1=cy*x-sy*y,y1=sy*x+cy*y;return[x1,cp*y1-sp*z,sp*y1+cp*z];}})}}
