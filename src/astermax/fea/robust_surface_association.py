...
def score_planar_cad_match(descriptor: SurfaceDescriptorV1, signature, model_diagonal_mm: float) -> tuple[float, dict]:
    ...
    bbox = np.asarray(descriptor.bbox_mm)
    cad_bbox = np.asarray(signature.bbox_mm)
    centroid = np.asarray(descriptor.centroid_mm)
    cad_centroid = np.asarray(signature.center_mm)
    bbox_error = float(np.linalg.norm(bbox - cad_bbox) / diagonal)
    centroid_error = float(np.linalg.norm(centroid - cad_centroid) / diagonal)
    area_error = abs(descriptor.area_mm2 - float(signature.area_mm2)) / max(abs(float(signature.area_mm2)), 1e-12)
    spans = cad_bbox[3:] - cad_bbox[:3]
    normal_error = 0.0
    if str(signature.surface_type).upper().startswith("PLANE"):
        expected = np.zeros(3); expected[int(np.argmin(np.abs(spans)))] = 1.0
        normal_error = float(np.linalg.norm(np.asarray(descriptor.normal_abs) - expected))
    metrics = {"bbox_error_rel": bbox_error, "centroid_error_rel": centroid_error, "area_error_rel": area_error, "normal_error": normal_error}
    score = bbox_error / 1e-6 + centroid_error / 1e-6 + area_error / 5e-3 + normal_error / 2e-3
    return float(score), metrics


def choose_unique_cad_face(descriptor: SurfaceDescriptorV1, cad_faces, model_diagonal_mm: float, uniqueness_margin_min: float = 0.15):
    ranked = []
    ...
    best = ranked[0]
    if best[2]["bbox_error_rel"] > 1e-6 or best[2]["centroid_error_rel"] > 1e-6 or best[2]["area_error_rel"] > 5e-3 or best[2]["normal_error"] > 2e-3:
