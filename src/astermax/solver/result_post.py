...
class FeResultTopologyManifestV1(BaseModel):
    ...
    schema_version: str = Field(default="FeResultTopologyManifestV1", pattern=r"^FeResultTopologyManifestV1$")
    mesh: MeshLevelV1
...
class MeshConvergenceSampleV1(BaseModel):
    ...
    schema_version: str = Field(default="MeshConvergenceSampleV1", pattern=r"^MeshConvergenceSampleV1$")
    mesh: MeshLevelV1
...
class ConvergenceThresholdsV1(BaseModel):
    model_config = ConfigDict(extra="forbid")
    schema_version: str = Field(default="ConvergenceThresholdsV1", pattern=r"^ConvergenceThresholdsV1$")
...
class MeshConvergenceReportV1(BaseModel):
    model_config = ConfigDict(extra="forbid")
    schema_version: str = Field(default="MeshConvergenceReportV1", pattern=r"^MeshConvergenceReportV1$")
    coarse_level: str
    fine_level: str
...
def _metric(name: str, coarse: float, fine: float, threshold: float, *, gate_metric: bool = True) -> ConvergenceMetricResultV1:
...
def evaluate_pair(coarse: MeshConvergenceSampleV1, fine: MeshConvergenceSampleV1, thresholds: ConvergenceThresholdsV1 | None = None) -> MeshConvergenceReportV1:
    ...
    metrics = [
        _metric("displacement_probe_mm", coarse.displacement_probe_mm, fine.displacement_probe_mm, thresholds.displacement_fraction),
        _metric("reaction_resultant_n", coarse.reaction_resultant_n, fine.reaction_resultant_n, thresholds.reaction_fraction),
        _metric("contact_resultant_n", coarse.contact_resultant_n, fine.contact_resultant_n, thresholds.contact_resultant_fraction),
        _metric("active_contact_area_mm2", coarse.active_contact_area_mm2, fine.active_contact_area_mm2, thresholds.active_contact_area_fraction),
        _metric("von_mises_p95_mpa", coarse.von_mises_p95_mpa, fine.von_mises_p95_mpa, thresholds.von_mises_percentile_fraction),
        _metric("von_mises_p99_mpa", coarse.von_mises_p99_mpa, fine.von_mises_p99_mpa, thresholds.von_mises_percentile_fraction),
        _metric("contact_pressure_p95_mpa", coarse.contact_pressure_p95_mpa, fine.contact_pressure_p95_mpa, thresholds.contact_pressure_percentile_fraction),
        _metric("contact_pressure_p99_mpa", coarse.contact_pressure_p99_mpa, fine.contact_pressure_p99_mpa, thresholds.contact_pressure_percentile_fraction),
        _metric("von_mises_max_mpa", coarse.von_mises_max_mpa, fine.von_mises_max_mpa, 1.0, gate_metric=False),
        _metric("contact_pressure_max_mpa", coarse.contact_pressure_max_mpa, fine.contact_pressure_max_mpa, 1.0, gate_metric=False),
    ]
...
def evaluate_ladder(samples: Iterable[MeshConvergenceSampleV1], thresholds: ConvergenceThresholdsV1 | None = None) -> list[MeshConvergenceReportV1]:
    ordered = list(samples)
    if len(ordered) < 2:
        raise ValueError("at least two mesh levels are required for convergence evaluation")
    return [evaluate_pair(ordered[index], ordered[index + 1], thresholds) for index in range(len(ordered) - 1)]
