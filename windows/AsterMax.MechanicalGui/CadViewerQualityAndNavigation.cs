...
internal static class CadViewerQualityBootstrap
{
    ...
    internal static void Start()
    {
        if (_started) return;
        _started = true;
        _monitor = new System.Windows.Forms.Timer { Interval = 250 };
        _monitor.Tick += (_, _) =>
        {
            try { Tick(); }
            catch { /* optional viewer services must never terminate AsterMax */ }
        };
        _monitor.Start();
    }
    ...
    private static void Tick()
    {
        ...
        CloseCompletedProgressWindows();
    }

    private static void EnsureAdvancedViewer(SelectableCadMeshCanvas legacy)
    {
        if (legacy.IsDisposed || legacy.Parent is null || !legacy.IsHandleCreated) return;
    ...
    private static void CloseCompletedProgressWindows()
    {
        var mechanical = Application.OpenForms.Cast<Form>().OfType<MechanicalForm>().FirstOrDefault();
        if (mechanical is null) return;
        var previewReady = GetField<CadMesh>(mechanical, "_cadSurfacePreview") is not null;
        var busy = GetField<bool>(mechanical, "_busy");
        ...
        foreach (Form form in Application.OpenForms.Cast<Form>().ToArray())
        {
            ...
            if (!ProgressStarted.TryGetValue(form, out var started))
                ProgressStarted[form] = started = DateTime.UtcNow;
            ...
            else if (DateTime.UtcNow - started > TimeSpan.FromSeconds(90))
            {
                form.BeginInvoke(form.Close);
                ProgressStarted.Remove(form);
            }
...
internal sealed class AdvancedCadViewport : Control
{
    ...
    internal AdvancedCadViewport(SelectableCadMeshCanvas legacy)
    {
        _legacy = legacy;
        DoubleBuffered = true;
    ...
    {
        base.OnMouseWheel(e);
        _zoom = Math.Clamp(_zoom * (e.Delta > 0 ? 1.12 : 1.0 / 1.12), 0.08, 18.0);
        Invalidate();
    }
    ...
    protected override void OnMouseMove(MouseEventArgs e)
    {
        ...
        {
            _yaw += dx * 0.009;
            _pitch = Math.Clamp(_pitch + dy * 0.009, -Math.PI * 0.495, Math.PI * 0.495);
            _last = e.Location;
            Invalidate();
        }
    ...
    protected override void OnMouseUp(MouseEventArgs e)
    {
        ...
        Capture = false;
        if (!wasDragging && e.Button == MouseButtons.Left && !_cubeBounds.Contains(e.Location) && !_meshButtonBounds.Contains(e.Location))
            SelectAt(e.Location);
    }
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        using var background = new System.Drawing.Drawing2D.LinearGradientBrush(ClientRectangle, Color.White, Color.FromArgb(218, 231, 243), 90f);
        g.FillRectangle(background, ClientRectangle);
        DrawFloor(g);
        _hits.Clear();
        if (_mesh is null || _topology is null || Width <= 1 || Height <= 1) return;

        var center = (_mesh.Min + _mesh.Max) / 2.0;
        var size = _mesh.Max - _mesh.Min;
        ...
        (PointF Point, double Depth, Vec3 NormalView) Project(Vec3 original, Vec3 normal)
        {
            ...
            var y2 = y1 * cp - z1 * sp;
            var z2 = y1 * sp + z1 * cp;
            var nx1 = normal.X * cy - normal.Y * sy;
            var ny1 = normal.X * sy + normal.Y * cy;
            var nz1 = normal.Z;
            var ny2 = ny1 * cp - nz1 * sp;
            var nz2 = ny1 * sp + nz1 * cp;
        ...
        {
            var tri = _mesh.SurfaceTriangles[i];
            var a = _mesh.Nodes[tri[0]];
            var b = _mesh.Nodes[tri[1]];
            var c = _mesh.Nodes[tri[2]];
            var normal = Normalize(Cross(b - a, c - a));
            var pa = Project(a, normal); var pb = Project(b, normal); var pc = Project(c, normal);
            var tag = i < _topology.TriangleFaceTags.Length ? _topology.TriangleFaceTags[i] : 0;
            rendered.Add(new RenderTriangle(tag, new[] { pa.Point, pb.Point, pc.Point }, (pa.Depth + pb.Depth + pc.Depth) / 3.0, pa.NormalView));
        }
        rendered.Sort((a, b) => a.Depth.CompareTo(b.Depth));
        var light = Normalize(new Vec3(-0.35, -0.55, 0.76));
        using var meshPen = new Pen(Color.FromArgb(95, 31, 57, 76), 0.55f);

        foreach (var item in rendered)
        {
            var illumination = Math.Clamp(0.42 + 0.58 * Math.Abs(Dot(item.NormalView, light)), 0.30, 1.0);
            ...
            g.FillPolygon(brush, item.Points);
            if (_volumeMesh && _showMesh) g.DrawPolygon(meshPen, item.Points);
            _hits.Add(new HitTriangle(item.FaceTag, item.Points, item.Depth));
        }

        DrawHeader(g);
        DrawNavigationCube(g);
        DrawMeshButton(g);
    }
    ...
    private void DrawHeader(Graphics g)
    {
        ...
        using var text = new Font("Segoe UI", 8.4f);
        g.DrawString(_volumeMesh ? "Geometría CAD + superposición de malla" : "Geometría CAD suavizada", title, Brushes.DarkSlateGray, 27, 24);
        g.DrawString($"{_mesh.Nodes.Count:N0} nodos · {_mesh.SurfaceTriangles.Count:N0} caras · {_mesh.Tetrahedra.Count:N0} TET4", text, Brushes.SlateGray, 27, 48);
    ...
    {
        _meshButtonBounds = new Rectangle(Math.Max(0, Width - 126), 126, 104, 30);
        ...
        g.FillRectangle(brush, _meshButtonBounds);
        g.DrawRectangle(border, _meshButtonBounds);
    ...
    {
        _cubeBounds = new Rectangle(Math.Max(0, Width - 132), 18, 108, 98);
        var center = new PointF(_cubeBounds.Left + 54, _cubeBounds.Top + 49);
        ...
        using var topBrush = new SolidBrush(Color.FromArgb(241, 246, 250));
        using var leftBrush = new SolidBrush(Color.FromArgb(207, 221, 233));
        using var rightBrush = new SolidBrush(Color.FromArgb(224, 235, 243));
        using var border = new Pen(Color.FromArgb(74, 99, 118), 1.1f);
        g.FillPolygon(topBrush, top); g.FillPolygon(leftBrush, left); g.FillPolygon(rightBrush, right);
        g.DrawPolygon(border, top); g.DrawPolygon(border, left); g.DrawPolygon(border, right);
        using var font = new Font("Segoe UI Semibold", 7.6f);
        g.DrawString("TOP", font, Brushes.DarkSlateGray, center.X - 14, center.Y - 21);
        g.DrawString("FRONT", font, Brushes.DarkSlateGray, center.X - 31, center.Y + 15);
        g.DrawString("RIGHT", font, Brushes.DarkSlateGray, center.X + 5, center.Y + 15);
    }

    private void ApplyCubeView(Point location)
    {
        var localX = location.X - _cubeBounds.Left;
        var localY = location.Y - _cubeBounds.Top;
    ...
    {
        using var pen = new Pen(Color.FromArgb(34, 91, 120, 145), 0.7f);
        var horizon = Height * 0.78f;
        for (var i = -14; i <= 14; i++)
        {
            var x = Width / 2f + i * 46f;
            g.DrawLine(pen, x, horizon - 60, x + i * 8, Height);
        }
    ...
    private static bool PointInTriangle(Point p, PointF a, PointF b, PointF c)
    {
        ...
        var q = new PointF(p.X, p.Y);
        var d1 = Sign(q, a, b); var d2 = Sign(q, b, c); var d3 = Sign(q, c, a);
