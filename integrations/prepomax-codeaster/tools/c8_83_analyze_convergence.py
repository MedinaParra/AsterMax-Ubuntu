...
root=pathlib.Path(sys.argv[1] if len(sys.argv)>1 else 'artifact/c8-83'); levels=['coarse','medium','fine']
...
results=[]
for name in levels:
    d=root/name; study=d/'study'; evp=d/'C8.83_SURFACE_TRACTION.json'
    ...
    fxarea=float(mm.group(1).replace('D','E'))
    if abs(fxarea-tev['traction_fx_mpa'])>1e-12:raise SystemExit(name+': traction COMM/evidence mismatch')
...
ev={'schema':'astermax.c8.83.surface-traction-convergence.v1','solver':'Code_Aster 17.4.0','unit_system':'mm-N-MPa','load_contract':'uniform +X surface traction, independently integrated to 1000 N','levels':results,'fine_pair_displacement_relative_change':du,'fine_pair_mises_relative_change':ds,'convergence_tolerance':tol,'displacement_convergence_admitted':du<=tol,'peak_mises_convergence_admitted':ds<=tol,'solution_convergence_admitted':du<=tol and ds<=tol,'all_reaction_balances_verified':True,'product_surface_traction_binding_verified':False,'qualification_surface_transform_verified':True,'mesh_quality_metric':'corner-node tetrahedral mean-ratio proxy; not a NetGen Jacobian metric','industrial_validation':False,'ansys_equivalence':False}
(root/'C8.83_CONVERGENCE_QUALIFICATION.json').write_text(json.dumps(ev,indent=2),encoding='utf-8')
with (root/'C8.83_CONVERGENCE.csv').open('w',newline='',encoding='utf-8') as f:
