...
Prerequisites: Git, Python 3, Visual Studio/.NET Framework toolchain required by the pinned PrePoMax revision, and a working Code_Aster installation (`as_run`) for real Code_Aster execution.
...
4. `apply_results_bridge.py` for `.resu/.rmed -> FeResults` post-processing integration.

GitHub Actions performs the same bootstrap, runs the harness self-test, restores upstream NuGet dependencies, verifies integration markers and compiles `PrePoMax.sln` in Release/x64.

## Current boundary / next layer

The current automated translator intentionally targets linear 3-D solid statics. Shells, beams, contacts, plasticity/nonlinear mechanics, modal/dynamic steps and thermal studies still require dedicated semantic translators before they should be generated automatically.

The next validation milestone is a **real solver smoke test**, not another mocked result path:

```text
PrePoMax model
   -> .mail/.comm/.export
   -> AsterMax Solver Harness
   -> real Code_Aster/as_run
   -> .mess/.rmed/.resu
   -> CodeAsterResultBridge
   -> CaeResults
   -> VTK result tree
...
After that passes, numerical regression contracts can be added to the harness (mesh fingerprint, reaction balance, energy checks and explicit reference-solution tolerances).
