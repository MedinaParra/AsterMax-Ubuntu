...
A second `Generated Code_Aster E2E` workflow extends the test boundary to include `FeModel -> CodeAsterModelTranslator.WriteStudy()` so the `.mail/.comm/.export` are generated by the integration itself before the real Code_Aster solve. Its first run is tracked independently from the already established real-solver/result-import proof above.
