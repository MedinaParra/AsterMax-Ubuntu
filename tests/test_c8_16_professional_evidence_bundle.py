...
def evidence(root: Path, **overrides):
    values = dict(
        ...
        mesh_quality_report_sha256="e" * 64,
        mesh_quality_artifact_sha256="f" * 64,
        reference_case_evidence_sha256="1" * 64,
...
def test_create_and_replay_verify_bundle(tmp_path: Path):
    ...
    bundle = create_professional_evidence_bundle(
        tmp_path,
        evidence(tmp_path),
        manifest,
        source_step=tmp_path / "reference.step",
    )
    ...
    assert {item.role for item in bundle.artifacts} >= {"source_step", "input_med", "result_med", "solver_message"}
...
def test_unverified_solve_cannot_be_packaged(tmp_path: Path):
    ...
    with pytest.raises(ProfessionalEvidenceBundleError, match="SOLVE_NOT_EXECUTED"):
        create_professional_evidence_bundle(
            tmp_path,
            evidence(tmp_path, fea_solve_executed=False, numerical_verification=False, results_verified=False),
