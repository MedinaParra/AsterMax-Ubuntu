...
report='''<!doctype html><html lang="es"><meta charset="utf-8"><title>AsterMax C10.10.1 — Auditoría</title>
...
<h1>AsterMax C10.10.1 — Auditoría del código y controles</h1><p>Fecha: 15 septiembre 2026. Inventario del código generado que se compila, incluyendo ventanas nativas y menús.</p>
...
<li>Iconos nativos con color por acción en todos los botones de cinta; nombres accesibles y tooltips; comando View → Auditoria.</li>
...
<li>Run todavía es síncrono: pendiente cancelación y ejecución asíncrona. Code_Aster se ejecuta mediante el backend nativo de Windows validado por Runtime/PREFLIGHT; WSL no forma parte del transporte C10.10.1.</li>
<li>Corregido: el validador de asignaciones ahora reconoce piezas (PartName) y grupos (ElementSetName), exige secciones sólidas activas/válidas y cobertura única. La exportación nativa bloquea elementos sin material asignado.</li>
<li>Corregido: Run rechaza tipos derivados no soportados (incluido SlipWear), estudios no lineales o inactivos, grupos de nodos inválidos y contactos/constraints/condiciones iniciales/campos definidos activos que antes podían quedar fuera del contrato.</li>
<li>C10.18: los resultados Code_Aster se alojan en el área gráfica principal y sus campos aparecen bajo Solution. Los botones de resultados y cámara se dirigen al visor activo. Las comprobaciones ejecutadas están en <a href="C10.18/UI_AUDIT.json">UI_AUDIT.json</a>; este inventario estático no sustituye esas pruebas.</li>
<li>Prueba Windows: inventario de botones con delegado e imagen, capturas por pestaña, árbol/cinta sin superposición, importación STEP y mallado reales. No ejecuta automáticamente comandos de edición o borrado. El detalle de esta ejecución está en Validation/C10.10.1/*.buttons.json.</li>
<li>Regresiones de configuración con modelos nativos aislados: asignaciones a pieza/grupo, material/sección ausentes o inactivos, duplicación de asignación, apoyos/cargas inactivos, nodos faltantes y tipos no soportados. Estos fixtures comprueban indicadores y exportación, no simulan ni contienen resultados FEA.</li>
...
<h2>Biblioteca de materiales</h2><p>Seis referencias editables de acero, inoxidables, aluminio y titanio. <a href="MATERIALS.html">Propiedades, fuentes y supuestos</a>. Se comprueban carga nativa, copia desde el diálogo, cancelación, conversión de unidades y asignación a piezas.</p><h2>Inventario completo</h2><input id="filter" placeholder="Buscar botón, ventana, manejador o estado…"><h3>Cinta</h3>'''+table(ribbon)+'<h3>Ventanas y menús nativos</h3>'+table(rows)+'''<script>document.getElementById('filter').oninput=function(){const q=this.value.toLowerCase();document.querySelectorAll('tbody tr').forEach(r=>r.hidden=!r.textContent.toLowerCase().includes(q));};</script></html>'''
