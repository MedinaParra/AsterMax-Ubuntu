...
if(-not $s.Contains('public string ModelFingerprintSha256 { get; private set; }')){
  ...
  $s=$s.Replace($anchor,$anchor + [Environment]::NewLine + '        public string ModelFingerprintSha256 { get; private set; }')
}

if(-not $s.Contains('b.ModelFingerprintSha256 = ((string)root["integrity"]?["model_fingerprint_sha256"] ?? "").ToLowerInvariant();')){
  $anchor='            b.StressUnit = (string)root["units"]["stress"];'
  if(-not $s.Contains($anchor)){throw 'C9.98 results loader anchor missing.'}
  $s=$s.Replace($anchor,$anchor + [Environment]::NewLine + '            b.ModelFingerprintSha256 = ((string)root["integrity"]?["model_fingerprint_sha256"] ?? "").ToLowerInvariant();')
}
...
if(-not $s.Contains('C9.98 requires results bound to an AsterMax FeModel SHA-256 fingerprint.')){
  ...
  $code=@'
            if (String.IsNullOrWhiteSpace(ModelFingerprintSha256) || ModelFingerprintSha256.Length != 64 ||
                ModelFingerprintSha256.Any(c => !((c >= '0' && c <= '9') || (c >= 'a' && c <= 'f'))))
...
if(-not $s.Contains('public void RequireCurrentModel(CaeModel.FeModel model)')){
  ...
  $code=@'
        public void RequireCurrentModel(CaeModel.FeModel model)
        {
            ...
            if (!String.Equals(ModelFingerprintSha256, active.Sha256, StringComparison.OrdinalIgnoreCase))
