...
def norm_id(s):
    s=str(s).strip(); m=re.fullmatch(r'[NEM](\d+)',s,re.I)
...
def main():
    ...
    report={'schema':'astermax-c987-production-export-fidelity/v1','passed':all(c.values()),'checks':c,'hashes':{'model':sha(a.model),'mail':sha(a.mail),'comm':sha(a.comm),'adapter':sha(a.adapter)},'scope':{'exporter':'native-windows/Export-AsterMaxCodeAster.ps1','solver_run':'NOT_RUN','fea_result_claim':'NONE','aster_identifier_contract':'nodes N#, elements E#; numeric model IDs are normalized only for semantic comparison','section_semantics':'HEXA8 solid: no separate shell/beam section required'},'known_gap':'Validated scope remains single-material HEXA8 linear static with one FixedBC and one CLoad; multi-material, shells/beams, sections, contacts, distributed loads and nonlinear/dynamic analyses are not yet covered.'}
