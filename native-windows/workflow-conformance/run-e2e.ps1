...
if(-not(Test-Path $session)) {
    [ordered]@{
        release='C10.20.7';pass=$false;partial=$true;
...
if($timedOut) { throw "C10.20.7 workflow conformance timed out after $TimeoutSeconds seconds; report was preserved." }
if($exitCode -ne 0) { throw "C10.20.7 native audit process failed before a clean completion. ExitCode=$exitCode" }
