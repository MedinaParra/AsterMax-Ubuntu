...
$heightMatches = [regex]::Matches($u, $heightPattern)
if($heightMatches.Count -lt 1) { throw 'C10.24 ribbon height anchor missing.' }
# The first Height assignment inside BuildAsterMaxTopChrome after "asterMaxRibbon" is the ribbon height.
...
$hm = [regex]::Match($tail, $heightPattern)
if(-not $hm.Success) { throw 'C10.24 ribbon height after control anchor missing.' }
$absStart = $ribbonPos + $hm.Index
$replacement = $hm.Groups['i'].Value + 'Height = 132,'
$u = $u.Substring(0,$absStart) + $replacement + $u.Substring($absStart + $hm.Length)

$paddingPattern = '(?m)^(?<i>\s*)Padding\s*=\s*new Point\(\d+,\s*\d+\),\s*
...
$tail = $u.Substring($ribbonPos)
$pm = [regex]::Match($tail, $paddingPattern)
if(-not $pm.Success) { throw 'C10.24 ribbon padding anchor missing.' }
$absStart = $ribbonPos + $pm.Index
$replacement = $pm.Groups['i'].Value + 'Padding = new Point(12, 5),'
$u = $u.Substring(0,$absStart) + $replacement + $u.Substring($absStart + $pm.Length)
