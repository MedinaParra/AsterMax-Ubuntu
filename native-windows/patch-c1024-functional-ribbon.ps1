...
# Ribbon container layout: target only the asterMaxRibbon declaration.
...
$heightMatch = [regex]::Match($tail, '(?m)^(?<i>\s*)Height\s*=\s*\d+,\s*$')
if(-not $heightMatch.Success) { throw 'C10.24 ribbon height anchor missing.' }
$abs = $ribbonPos + $heightMatch.Index
$repl = $heightMatch.Groups['i'].Value + 'Height = 132,'
$u = $u.Substring(0,$abs) + $repl + $u.Substring($abs + $heightMatch.Length)

$tail = $u.Substring($ribbonPos)
$paddingMatch = [regex]::Match($tail, '(?m)^(?<i>\s*)Padding\s*=\s*new Point\(\d+,\s*\d+\),\s*$')
if(-not $paddingMatch.Success) { throw 'C10.24 ribbon padding anchor missing.' }
$abs = $ribbonPos + $paddingMatch.Index
$repl = $paddingMatch.Groups['i'].Value + 'Padding = new Point(12, 5),'
$u = $u.Substring(0,$abs) + $repl + $u.Substring($abs + $paddingMatch.Length)
...
$connectionsOld = @'
            ribbon.TabPages.Add(BuildRibbonPage("Conexiones", new Control[] {
...
'@
$u = Replace-Required $u $connectionsOld $connectionsNew 'Conexiones'
