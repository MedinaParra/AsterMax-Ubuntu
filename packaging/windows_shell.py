...
class AsterMaxWindowsRoot(_ORIGINAL_TK):
    ...
    def _not_ready(self, feature: str) -> None:
        messagebox.showinfo(
            "AsterMax",
            f"{feature} is not enabled in the verified PMV yet. The command is visible so the desktop information architecture remains stable while capability is added behind harness gates.",
            parent=self,
        )
    ...
    def _build_menubar(self) -> None:
        ...
        file_menu.add_command(label="Abrir carpeta de resultados", command=lambda: self._select_tab_contains("Results"))
        file_menu.add_separator()
        ...
        edit_menu.add_command(label="Preferencias…", command=lambda: self._not_ready("Preferences"))
        ...
        model_menu.add_command(label="Contactos…", command=lambda: self._not_ready("Contact authoring"))
        ...
        mesh_menu.add_command(label="Inspección de calidad", command=lambda: self._select_tab_contains("Review"))
        ...
        help_menu.add_command(label="Comprobar instalación", command=lambda: messagebox.showinfo("AsterMax", "Runtime loaded correctly. Use the packaged self-test for numerical verification.", parent=self))
